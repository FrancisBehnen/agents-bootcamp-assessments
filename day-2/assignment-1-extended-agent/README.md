...
- 🧰 **Business tools** for searching products, product details, order status,
...
- 📜 **A serious system prompt**: role, tone of voice, tool policy,
  ...
  engineering day.
- 💬 **Short-term memory**, where the agent remembers the *current* conversation
  across multiple turns (a LangGraph *checkpointer* + `thread_id`).
- 📔 **Long-term memory**, where the agent saves durable facts about the customer
